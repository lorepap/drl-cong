...
def parse_traces_config():
    with open(os.path.join(context.src_dir, 'config/traces.yml')) as config:
        return yaml.load(config, Loader=yaml.BaseLoader)


def parse_training_config():
    with open(os.path.join(context.src_dir, 'config/train.yml')) as config:
        return yaml.load(config, Loader=yaml.FullLoader)


def parse_prod_config():
    with open(os.path.join(context.src_dir, 'config/prod.yml')) as config:
        return yaml.load(config, Loader=yaml.BaseLoader)
